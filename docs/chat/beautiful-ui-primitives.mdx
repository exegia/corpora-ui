...
description: "Streaming text, recommendation card, context cards, code block, filter and records tables, flowchart and insight cards — Beautiful-UI structure on Exegia tokens."
icon: "layout-grid"
---

Spec folder: [`specs/004-beautiful-ui-primitives`](https://github.com/exegia/corpora-ui/tree/dev/specs/004-beautiful-ui-primitives). Sketch masters: `Component / Streaming Text`, `Recommendation Card`, `Context Cards`, `Code Block`, `Filter Table`, `Records Table`, `Flowchart`, `Insight Cards` in the `exegia-ui` document (cloud `01945ba6-1dcb-4f55-bc46-c0cf4d09aa7c`). Every component carries its master name in a `@sketch` JSDoc tag.

## Problem

The presentation layer of an agent answer needs richer surfaces than prose: a streamed paragraph with citations, a proposal to accept, retrieved chunks, code, tables and a small flow. Beautiful-UI defines the structure and spacing; its CSS foundation would be a second styling system, so nothing is imported verbatim.

## Solution

- Atoms: `Tag`, `Signal`, `SourceChip`, `AvatarStack`, `Stat`, `FollowUpRow`.
- Components in `composed/chat/`: `StreamingText`, `RecommendationCard`, `ContextCards`, `CodeBlock`, `FilterTable`, `RecordsTable`, `Flowchart`, `InsightCards`.
- Stateful pieces use keyed Jotai families from `lib/keyed-atom.ts`: `streamingSourcesOpenAtom`, `filterTableFilterAtom`, `recordsTableSelectionAtom`.
- Every colour is an Exegia token; styling is Tailwind; no highlighter or table dependency.

## UI reference

<Frame caption="Beautiful-UI primitives — Light (Sketch frame)">
  <img src="/images/ai-presentation-light.png" alt="Beautiful-UI primitives light showcase" />
</Frame>
<Frame caption="Beautiful-UI primitives — Dark (Sketch frame)">
  <img src="/images/ai-presentation-dark.png" alt="Beautiful-UI primitives dark showcase" />
</Frame>

## Usage

```tsx
import { StreamingText, FilterTable, Flowchart } from "@corpora/ui"
...
<Flowchart nodes={[{ kind: "trigger", title: "New order created" }]} />
```

## Acceptance criteria

Copied from `specs/004-beautiful-ui-primitives/spec.md`. Each holds in light and dark.
...
- [#140](https://github.com/exegia/corpora-ui/issues/140)
- [#145](https://github.com/exegia/corpora-ui/issues/145)
- [#149](https://github.com/exegia/corpora-ui/issues/149)
- [#150](https://github.com/exegia/corpora-ui/issues/150)
