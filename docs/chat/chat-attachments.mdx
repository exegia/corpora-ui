...
- Blocks in `blocks/chat/`: `MessageSender`, `MessageRecipient`, `ComposerWithAttachments` — the tray lives in a keyed Jotai family (`composerAttachmentsAtom(id)`).
...
```tsx
import { Attachment, ComposerWithAttachments, MessageSender } from "@corpora/ui"
...
<ComposerWithAttachments composerId="thread-1" onSend={(draft, attachments) => send(draft, attachments)} />
```
