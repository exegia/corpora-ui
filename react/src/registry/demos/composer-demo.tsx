...
<Composer onSubmit={(draft) => send(draft)} />`}
      ...
      <Example
        title="Composer with attachments and a command menu"
        code={`import { CommandMenu, Composer, type ComposerAttachment } from "@corpora/ui"
...
<Composer
  ...
  ComposerMenu={() => (
    <CommandMenu items={commands} onCommand={(item) => pick(item.id)} />
  )}
  onSubmit={(draft, _mode, atts) => send(draft, atts)}
  safetyNote={null}
/>`}
      >
        <Composer
