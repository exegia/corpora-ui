...
describe("ProfileCardBlock", () => {
  ...
  test("variant=collapsed folds to the avatar but keeps the accessible name", () => {
    const { rerender } = render(
      <ProfileCardBlock user={USER} variant="collapsed" />
    )
    ...
    expect(screen.getByText("JH")).toBeDefined()
    const identity = trigger.querySelector(
  ...
  test("outside any panel an unnamed card starts expanded", () => {
    render(<ProfileCardBlock user={USER} />)
    expect(screen.getByRole("button").hasAttribute("data-collapsed")).toBe(
      false
    )
  ...
  test("user.presence and the presence prop reach the avatar badge", () => {
    ...
    // The atoms avatar badge carries the status as a color class, not a
    // data attribute (see atoms/avatar/utils statusClasses).
    const badge = () => document.querySelector('[data-slot="avatar-badge"]')
    expect(badge()?.classList.contains("bg-success")).toBe(true)
    rerender(
      <ProfileCardBlock
        presence="offline"
        user={{ ...USER, presence: "online" }}
      />
    )
    expect(badge()?.classList.contains("bg-text-secondary")).toBe(true)
