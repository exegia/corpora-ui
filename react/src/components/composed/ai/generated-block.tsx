...
export function GeneratedBlock({
  ...
  return (
    ...
    >
      ...
      <div className="text-sm leading-5 font-medium text-foreground/90">
        {content}
        {isStreaming ? (
          <>
            <span
              aria-hidden="true"
              className="ml-1 inline-block h-4 w-0.5 translate-y-0.5 animate-caret-blink bg-amber-400/80"
            />
            <Button
        ...
        <div className="mt-2 flex flex-wrap gap-1.5" aria-label="Citations">
          {citations.map((citation) => (
            <ReferenceChip
              ...
              key={citation}
              onClick={
                onCitationSelect ? () => onCitationSelect(citation) : undefined
              }
              showArrow={Boolean(onCitationSelect)}
            >
              {citation}
            </ReferenceChip>
          ))}
