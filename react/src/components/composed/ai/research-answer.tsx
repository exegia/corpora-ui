...
const GHOST =
  "h-6 gap-1.5 rounded-md px-2 text-[11.5px] font-medium text-text-secondary hover:bg-surface-subtle hover:text-text-primary sm:h-6 [&_svg]:size-3.5! [&_svg]:opacity-100"
...
export function ResearchAnswer({
  ...
  return (
    <CardFrame
      ...
      className={cn(
        "w-[380px] max-w-full [--frame-radius:var(--radius-md)]",
    ...
    >
      <CardFrameHeader className="flex flex-row items-start gap-2 px-3.5 py-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
          <span className="text-[13px] leading-4 font-semibold text-text-primary">
            {kicker}
          </span>
          {kickerSub ? (
      ...
      <Card>
        <CardPanel className="px-3.5 py-3.5 text-[13px] leading-[18px] text-text-primary">
          {content}
        </CardPanel>
        {meta.length ? (
          <>
            ...
            <dl className="flex flex-col gap-1.5 px-3.5 py-3">
              {meta.map((m) => (
                <div key={m.label} className="flex items-baseline gap-3">
                  <dt className="flex w-20 shrink-0 items-center gap-1 text-[11px] leading-4 text-text-secondary [&_svg]:size-3 [&_svg]:self-center [&_svg]:text-icon">
                    {m.icon}
                    {m.label}
                  </dt>
                  <dd className="min-w-0 flex-1 text-[12px] leading-4 text-text-primary">
                    {asPill(m.value)}
                  </dd>
                </div>
              ))}
      ...
      <CardFrameFooter className="flex items-center gap-0.5 p-2">
        ...
        <Button variant="ghost" size="xs" className={GHOST} onClick={onShare}>
          <Share2 />
          Share
        </Button>
        <Button
