...
<type>/<slug> ──PR──> dev ──(daily/manual)──> next ──cut──> release/vX.Y.Z ──draft PR──> main
                 (deleted on merge)         (pack)                          (deleted on release)
```

Same model as [corpora-web](https://github.com/exegia/corpora-web). This repo
ships as an npm package: where corpora-web deploys to Vercel, `main` here
publishes `@exegia/corpora-ui` with provenance.
...
packs an npm tarball as a gate.
...
Marking it ready for review runs the tests plus a real `npm pack`, uploaded as
an artifact — if the tarball cannot be produced there, the production publish
would have failed after the merge, on protected `main`.
...
release branch, then opens PRs that merge `main` back into `next` and `dev` and
deletes leftover remote feature / `release/v*` heads.
...
| File                          | Trigger                    | Does                                                     |
|-------------------------------|----------------------------|----------------------------------------------------------|
| `pr.yml`                      | PR opened / ready / pushed | `guard`, `check`, `package` (into main), `review` (into `dev` or `release/v*`) |
| `promote.yml`                 | 22:00 UTC daily / manual   | bootstrap lanes, open `dev` → `next` PR, auto-merge      |
| `next.yml`                    | push to `next`             | pack tarball, cut/refresh `release/v*`                   |
| `pr-merged.yml`               | push to `release/v*`       | upsert the draft release PR into `main`                  |
| `release.yml`                 | PR merged into `main`      | publish to npm, tag, sync lanes, cleanup                 |
| `publish-github-packages.yml` | PR merged into `main`      | mirror the release to GitHub Packages                    |
| `opencode.yml`                | `/oc` comment              | repo-specific agent trigger                              |

Every step is a `make` target, so anything CI does can be reproduced locally.

The production publish lives inside `release.yml` rather than hanging off the
...
(`Actions → Release → Run workflow`) — the publish job skips and wrapup creates
the lanes. Locally:
...
or a tag already released is skipped, not an error. `make rulesets-apply`
matches by `.name`, so the file must keep the name of the ruleset already on
GitHub (`main-no-direct-push`, `Protect dev branch`, `Protect next branch`) or
a second one is created alongside it.
